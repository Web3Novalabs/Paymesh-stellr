...
/// Each deposit is appended to the group's deposit history in order.
#[test]
...
/// Depositor history spans multiple groups correctly.
#[test]
fn test_depositor_history_spans_multiple_groups() {
    ...
    let history = client.get_depositor_history(&depositor);
    assert_eq!(history.len(), 2);
    assert_eq!(history.get(0).unwrap().group_id, id_a);
    assert_eq!(history.get(1).unwrap().group_id, id_b);
}
