...
#[test]
fn test_update_members_success_bottom() {
    let env = Env::default();
    let contract_id = env.register(AutoShareContract, ());
    let client = AutoShareContractClient::new(&env, &contract_id);
    ...
    let name = String::from_str(&env, "Group With Members");

    // create expects members argument in current interface
    let member_init = GroupMember {
        address: Address::generate(&env),
        percentage: 100,
    };
    let init_members = Vec::from_array(&env, [member_init]);

    client.create(&id, &name, &creator, &init_members);
...
#[test]
#[should_panic]
fn test_update_members_unauthorized_bottom() {
    let env = Env::default();
    let contract_id = env.register(AutoShareContract, ());
    let client = AutoShareContractClient::new(&env, &contract_id);
    ...
    let id = BytesN::from_array(&env, &[43u8; 32]);
    let name = String::from_str(&env, "Group Unauthorized");
    ...
    client.update_members(&id, &other, &members);
}

#[test]
#[should_panic]
fn test_update_members_nonexistent_autoshare_bottom() {
    let env = Env::default();
    let contract_id = env.register(AutoShareContract, ());
    let client = AutoShareContractClient::new(&env, &contract_id);

    let creator = Address::generate(&env);
    let id = BytesN::from_array(&env, &[44u8; 32]);
    ...
    // no create call before update -> should panic NotFound
    client.update_members(&id, &creator, &members);
}

#[test]
#[should_panic]
fn test_update_members_invalid_percentages_bottom() {
    let env = Env::default();
    let contract_id = env.register(AutoShareContract, ());
    let client = AutoShareContractClient::new(&env, &contract_id);

    let creator = Address::generate(&env);
    let id = BytesN::from_array(&env, &[45u8; 32]);
    let name = String::from_str(&env, "Group Invalid Percentages");
...
#[test]
#[should_panic]
fn test_update_members_empty_members_bottom() {
    let env = Env::default();
    let contract_id = env.register(AutoShareContract, ());
    let client = AutoShareContractClient::new(&env, &contract_id);

    let creator = Address::generate(&env);
    let id = BytesN::from_array(&env, &[46u8; 32]);
    let name = String::from_str(&env, "Group Empty Members");
    ...
    let members = Vec::<GroupMember>::new(&env);
    client.update_members(&id, &creator, &members);
}

#[test]
#[should_panic]
fn test_update_members_duplicate_addresses_bottom() {
    let env = Env::default();
    let contract_id = env.register(AutoShareContract, ());
    let client = AutoShareContractClient::new(&env, &contract_id);

    let creator = Address::generate(&env);
    let id = BytesN::from_array(&env, &[47u8; 32]);
    let name = String::from_str(&env, "Group Duplicate Members");
