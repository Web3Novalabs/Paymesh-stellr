...
fn test_get_contributions_paginated_empty() {
    ...
    let (g_contributions, g_total) = client.get_group_contributions_paginated(&group_id, &0, &10);
    let (u_contributions, u_total) = client.get_user_contributions_paginated(&user, &0, &10);
...
fn test_get_group_contributions_pagination() {
    ...
    let (page1, total) = client.get_group_contributions_paginated(&group_id, &0, &10);
    ...
    let (page2, total) = client.get_group_contributions_paginated(&group_id, &10, &10);
    ...
    let (page3, total) = client.get_group_contributions_paginated(&group_id, &20, &10);
    ...
    let (capped_page, total) = client.get_group_contributions_paginated(&group_id, &0, &50);
    assert_eq!(capped_page.len(), 20);
    assert_eq!(total, 25);

    // Test offset out of bounds: offset 30, limit 10
    let (empty_page, total) = client.get_group_contributions_paginated(&group_id, &30, &10);
...
fn test_get_user_contributions_pagination() {
    ...
    // Test page 1: offset 0, limit 10
    let (page1, total) = client.get_user_contributions_paginated(&user, &0, &10);
    assert_eq!(page1.len(), 10);
    assert_eq!(total, 15);

    // Test page 2: offset 10, limit 10
    let (page2, total) = client.get_user_contributions_paginated(&user, &10, &10);
