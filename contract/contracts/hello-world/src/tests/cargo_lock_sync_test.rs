...
mod cargo_lock_sync_tests {
    ...
    fn test_bug_condition_locked_flag_causes_failure() {
        ...
        let cargo_toml_backup = fs::read_to_string(&cargo_toml_path)
            .expect("Failed to read Cargo.toml");
        let cargo_lock_backup = fs::read_to_string(&cargo_lock_path)
        ...
        fs::write(&cargo_toml_path, cargo_toml_backup)
            .expect("Failed to restore Cargo.toml");
        fs::write(&cargo_lock_path, cargo_lock_backup)
            .expect("Failed to restore Cargo.lock");
